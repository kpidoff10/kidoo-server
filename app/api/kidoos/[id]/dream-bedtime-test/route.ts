/**
 * Route API pour tester la configuration de l'heure de coucher du modèle Dream
 * POST /api/kidoos/[id]/dream-bedtime-test - Démarre ou arrête le test
 */
...
/**
 * POST /api/kidoos/[id]/dream-bedtime-test
 * Démarre ou arrête le test de l'heure de coucher
 */
export const POST = withAuth(async (
  ...
  try {
    ...
    if (action === 'stop') {
      // Arrêter le test
      const sent = await sendCommand(kidoo.macAddress, 'stop-test-bedtime');
    ...
    console.log('[DREAM-BEDTIME-TEST] Paramètres reçus:', { color, brightness });
    ...
    if (color && typeof color === 'string' && color.trim() !== '' && brightness !== undefined && brightness !== null) {
      ...
      if (!rgb) {
        console.error('[DREAM-BEDTIME-TEST] Couleur hex invalide:', color);
        return createErrorResponse('VALIDATION_ERROR', 400, {
      ...
      console.log('[DREAM-BEDTIME-TEST] Utilisation des paramètres fournis:', {
        colorHex: color,
      ...
      testColorR = kidoo.configDream.colorR ?? 255;
      testColorG = kidoo.configDream.colorG ?? 107;
      testColorB = kidoo.configDream.colorB ?? 107;
      testBrightness = kidoo.configDream.brightness ?? 50;
      
      console.log('[DREAM-BEDTIME-TEST] Utilisation de la configuration sauvegardée:', {
      ...
      console.log('[DREAM-BEDTIME-TEST] Utilisation des valeurs par défaut:', {
        rgb: { r: testColorR, g: testColorG, b: testColorB },
        brightness: testBrightness,
      });
    }
    ...
    console.log('[DREAM-BEDTIME-TEST] Envoi commande PubNub avec:', {
      colorR: testColorR,
      colorG: testColorG,
      colorB: testColorB,
      brightness: testBrightness,
    ...
    const sent = await sendCommand(kidoo.macAddress, 'start-test-bedtime', {
      params: {
        colorR: testColorR,
        colorG: testColorG,
        colorB: testColorB,
    ...
    console.error('Erreur lors du test de l\'heure de coucher:', error);
    return createErrorResponse('INTERNAL_ERROR', 500, {
